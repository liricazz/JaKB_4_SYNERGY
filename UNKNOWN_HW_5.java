...
public class HomeWork7 {
    public static List<String> methodOne(List<String> names) {
        return names.stream().filter(name -> name.contains("King")).collect(Collectors.toList());
    }
    public static List<String> methodTwo(List<String> names) {
        return names.stream().map(name -> name.split(" ")[1]).distinct().collect(Collectors.toList());
    }
    public static List<String> methodThree(List<String> names) {
        return names.stream().map(name -> name.split(" ")[0]).filter(name -> name.startsWith("L")).distinct().collect(Collectors.toList());
    }
    public static List<String> methodFour(List<String> names) {
        return names.stream().map(name -> name.split(" ")[1]).filter(name -> name.startsWith("W")).distinct().sorted().collect(Collectors.toList());
    }
    public static Integer methodFive(List<String> names) {
        return (int)names.stream().map(name -> name.split(" ")[0]).filter(name -> name.length() > 6).distinct().count();
    }
    public static Integer methodSix(List<String> names) {
        return (int) names.stream().map(name -> name.substring(name.indexOf(" "))).filter(name -> name.length() > 5).distinct().count() + (int) names.stream().map(name -> name.substring(0, name.indexOf(" ")-1)).filter(name -> name.length() > 5).distinct().count();
    }
    public static List<String> methodSeven(List<String> names) {
        return names.stream().filter(name -> name.charAt(name.lastIndexOf(' ') + 1) == 'K' || name.charAt(name.lastIndexOf(' ') + 1) == 'S').sorted().distinct().map(name -> name.substring(0, name.indexOf(' ')) + " " + name.charAt(name.lastIndexOf(' ') + 1) + ".").distinct().collect(Collectors.toList());
    }
